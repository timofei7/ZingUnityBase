...
public class TTMonoBehaviour : MonoBehaviour
{
	...
	/// <summary>
	/// Starts a new style coroutine that does exception handling but DROPS the exceptions...
	/// </summary>
	//[System.Obsolete("Use StartCoroutine<T> instead as TTMonoBehaviour.StartCoroutine just DROPS exceptions on the flo.")]
	public new Coroutine StartCoroutine(IEnumerator coroutine) { return StartCoroutine<bool>(coroutine).coroutine; }
  ...
  // So you can call things without using strings but then kill them or whatever by string clever
